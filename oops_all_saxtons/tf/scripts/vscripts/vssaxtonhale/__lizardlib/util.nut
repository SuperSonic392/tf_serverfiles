...
::CreateAoE <- function(originPlayer, center, radius, applyDamageFunc, applyPushFunc) //midly edited
{
    foreach(target in GetAlivePlayers())
    {
        if(target.GetTeam() == originPlayer.GetTeam())
        {
            continue;
        }
...
::CreateAoEAABB <- function(originPlayer, center, min, max, applyDamageFunc, applyPushFunc)
{
