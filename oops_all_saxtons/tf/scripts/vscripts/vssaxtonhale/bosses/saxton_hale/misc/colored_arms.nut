...
function OnDiscard()
{
    // Clean up red arm wearable (global lookup-based, so must be destroyed defensively)
    local red = Entities.FindByName(null, "wearable_vs_hale_aura_red");
    if (red != null && red.IsValid())
        red.Kill();

    // Clean up blue arm wearable
    local blue = Entities.FindByName(null, "wearable_vs_hale_aura_blue");
