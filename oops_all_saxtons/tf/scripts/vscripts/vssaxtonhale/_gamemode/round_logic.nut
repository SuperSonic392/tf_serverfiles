...
AddListener("setup_start", 0, function ()
{
    ...
    {
        if(player.GetTeam() == TF_TEAM_BLUE && API_GetBool("enable_robot_saxton"))
        {
            AssignBoss("mecha_hale", player);
        }
        else
        {
            //AssignBoss("that_spy", player);
            AssignBoss("saxton_hale", player);
        }
    }
...
AddListener("death", 0, function(attacker, victim, params)
{
    ...
    pendingSaxtons[victim.entindex()] <- true;
});
...
//
    ...
    if (idx in pendingSaxtons)
    {
        delete pendingSaxtons[idx];
        bosses[player].TryApply(player);
    }
    else if(bosses[player] == null)
    {
        if(player.GetTeam() == TF_TEAM_BLUE && API_GetBool("enable_robot_saxton"))
        {
            AssignBoss("mecha_hale", player);
...
AddListener("bossify", -1, function(player, boss_name)
{
});
